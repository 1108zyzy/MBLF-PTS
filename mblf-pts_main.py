...
class DualMem(nn.Module):
    ...
    def update_memory_bank(self, model, target):
        ...
        if model.image_feature_count[pseudo_label] == model.memory_size:
            ...
            model.image_feature_memory[pseudo_label][
                model.image_feature_count[pseudo_label, 0].item()] = selected_image_features_global
            model.image_prediction_mem[pseudo_label][model.image_feature_count[pseudo_label, 0].item()] = mean_prob[0]
            model.image_entropy_mem[pseudo_label][
                model.image_feature_count[pseudo_label, 0].item()] = current_instance_entropy
            model.image_feature_count[pseudo_label] += 1

    def get_image_pred(self, model, return_full=False, return_logit=False, merged_image_feat=None):
...
def init_image_memory(train_loader, model, args):
    ...
    if getattr(args, 'n_shot', 0):
        if hasattr(model, 'prepare_sakr_bank'):
            model.prepare_sakr_bank()
...
def direct_inference(val_loader, model, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            if getattr(args, 'tda', False):
                with torch.no_grad():
                    # 文本分支 logits/prob/entropy
                    img_text_logits = dmnet.get_text_prediction(model, return_full=True, return_logit=True)  # [B, C]
                    clip_logits_1 = img_text_logits[:1]  # [1, C]
                    clip_prob_1 = clip_logits_1.softmax(dim=1)  # [1, C]
                    entropy_1 = -(clip_prob_1 * (clip_prob_1 + 1e-8).log()).sum(dim=1).item()
                    pred_idx = int(clip_prob_1.argmax(dim=1).item())
                    feat_1d = model.image_features_global[:1].detach().squeeze(0).cpu()  # [D on CPU]

                # 正缓存（收集“更确定”的样本：低熵优先）
                if args.tda_pos_k > 0:
                    bucket = pos_cache.setdefault(pred_idx, [])
                    item = (feat_1d, entropy_1)
                    if len(bucket) < args.tda_pos_k:
                        bucket.append(item)
                    else:
                        bucket.sort(key=operator.itemgetter(1))  # 按熵升序
                        if entropy_1 < bucket[-1][1]:
                            bucket[-1] = item

                # 负缓存（只收“中等不确定”的样本：τ_l < H < τ_h），并存其概率向量用于负掩码
                if args.tda_neg_k > 0 and (args.tda_neg_entropy_low < entropy_1 < args.tda_neg_entropy_high):
                    bucket = neg_cache.setdefault(pred_idx, [])
                    item = (feat_1d, entropy_1, clip_prob_1.squeeze(0).detach().cpu())  # [C on CPU]
                    if len(bucket) < args.tda_neg_k:
                        bucket.append(item)
                    else:
                        bucket.sort(key=operator.itemgetter(1))
                        if entropy_1 < bucket[-1][1]:
                            bucket[-1] = item


                def _compute_cache_logits(image_feat_1xD, cache_dict, alpha, beta, num_classes, neg_mask=None):
                    if not cache_dict:
                        return torch.zeros_like(clip_logits_1)
                    keys, values = [], []
                    for cls in sorted(cache_dict.keys()):
                        for it in cache_dict[cls]:
                            k = it[0].unsqueeze(1)  # [D,1]
                            keys.append(k)
                            if neg_mask is None:
                                one_hot = torch.zeros(num_classes, device=image_feat_1xD.device,
                                                      dtype=image_feat_1xD.dtype)
                                one_hot[cls] = 1.0
                                values.append(one_hot.unsqueeze(0))  # [1,C]
                            else:
                                low, high = neg_mask
                                prob_vec = it[2]  # [C]
                                m = ((prob_vec > low) & (prob_vec < high)).to(image_feat_1xD.dtype)
                                values.append(m.unsqueeze(0))  # [1,C]
                    device = image_feat_1xD.device
                    dtype = image_feat_1xD.dtype
                    Kmat = torch.cat(keys, dim=1).to(device=device, dtype=dtype)  # [D, K]
                    Vmat = torch.cat(values, dim=0).to(device=device, dtype=dtype)  # [K, C]
                    aff = image_feat_1xD @ Kmat  # [1,K]
                    adap = torch.exp(-beta * (1.0 - aff))  # [1,K]   A(z)=exp(-β(1-z))
                    logits = adap @ Vmat  # [1,C]
                    return alpha * logits

                with torch.no_grad():
                    num_classes = int(model.text_feat.shape[0])
                    pos_logits = _compute_cache_logits(model.image_features_global[:1], pos_cache,
                                                       getattr(args, 'tda_pos_alpha', 1.0),
                                                       getattr(args, 'tda_pos_beta', 5.0),
                                                       num_classes, neg_mask=None)
                    neg_logits = _compute_cache_logits(model.image_features_global[:1], neg_cache,
                                                       getattr(args, 'tda_neg_alpha', 1.0),
                                                       getattr(args, 'tda_neg_beta', 5.0),
                                                       num_classes,
                                                       neg_mask=(getattr(args, 'tda_neg_mask_low', 0.0),
                                                                 getattr(args, 'tda_neg_mask_high', 0.3)))
                    tda_total_logits = clip_logits_1 + pos_logits - neg_logits  # [1,C]
                    tda_pred.append(tda_total_logits.softmax(dim=1).detach().float().cpu())


        img_text_pred = img_text[:1]  ## current prediction.
        confidence_prediction, selected_idx, confused_weak_output, confused_idx = select_confident_samples(img_text,
                                                                                                           args.selection_p)
        # ipdb.set_trace()
        ...
        if args.n_shot:
            with torch.no_grad():
                ...
                confidence_prediction_fewshot_global, _, _, _ = select_confident_samples(fewshot_global_pred_fullview,
                                                                                         1.0)
        ...
        if getattr(args, 'use_sakr_branch', False) and getattr(args, 'n_shot', 0):
            with torch.no_grad():
                # 当前样本的全局图像特征 [1, D]（已 L2 归一）
                feats_1xD = model.image_features_global[:1]
                sakr_logit = model.sakr_logits_from_feats(
                    feats_1xD,
                    beta=getattr(args, 'sakr_beta', 5.0),
                    lmbda=getattr(args, 'sakr_lambda', 1.0)
                )  # [1, C]
                sakr_pred = sakr_logit.softmax(dim=1)  # [1, C]
        else:
            sakr_pred = img_text_pred

        # # ===== 根据数据集选择记忆写入策略 =====
        # sid = str(getattr(args, 'set_id', '')).strip()
        # is_shift = (sid.upper() in {'A', 'V', 'R', 'K'}) or \
        #            (isinstance(getattr(model, 'dataset_id', None), str) and model.dataset_id.lower() in {'a', 'v', 'r',
        #                                                                                                  'k'})
        # if is_shift:
        #     # ImageNet-A/R/V/K：只走简洁的动态写库
        #     dmnet.update_memory_bank(model, target)  # shift时候走这个，其他的走下面
        # else:
        #     # ================== EuroSAT-only gating（插在 boosting 写库块之前） ==================
        #
        #     # 稳定识别 EuroSAT：优先用 model.dataset_id
        #     is_eurosat = False
        #     if 'dataset_id' in dir(model) and isinstance(model.dataset_id, str):
        #         is_eurosat = ('eurosat' in model.dataset_id)
        #     # 兜底：再看 args.datasets
        #     if (not is_eurosat) and hasattr(args, 'datasets'):
        #         is_eurosat = ('eurosat' in str(args.datasets).lower())
        #
        #     if is_eurosat:
        #         # 你原来的 EuroSAT 门控逻辑原样保留：
        #         # 1) 共识+边距门控
        #         vals, idx = confidence_prediction.topk(2, dim=1)  # [n_views, 2]
        #         margins = vals[:, 0] - vals[:, 1]  # [n_views]
        #         view_preds = idx[:, 0]  # [n_views]
        #         major_label = view_preds.mode().values.item()
        #         agree_ratio = (view_preds == major_label).float().mean().item()
        #
        #         AGREE_MIN = getattr(args, 'eurosat_agree_min', 0.70)
        #         MARGIN_MIN = getattr(args, 'eurosat_margin_min', 0.20)
        #
        #         gate_mask = ((view_preds == major_label) & (margins >= MARGIN_MIN))
        #         selected_idx = torch.nonzero(gate_mask, as_tuple=False).squeeze(-1)
        #
        #         if agree_ratio < AGREE_MIN or selected_idx.numel() == 0:
        #             selected_idx = torch.tensor([], device=confidence_prediction.device, dtype=torch.long)
        #
        #         # 2) 多样性门控
        #         def _pass_diversity(j, cls_id, delta=0.05):
        #             mem = model.image_feature_memory[cls_id]
        #             cnt = int(model.image_feature_count[cls_id, 0].item())
        #             if cnt == 0:
        #                 return True
        #             feat = model.image_features_global[j]  # [D]
        #             mem = mem[:cnt]  # [m, D]
        #             max_cos = torch.max(F.cosine_similarity(mem, feat[None, :], dim=1)).item()
        #             return (1.0 - max_cos) >= getattr(args, 'eurosat_div_delta', 0.05)
        #
        #         kept = []
        #         cls_id = major_label
        #         for j in selected_idx.tolist():
        #             if _pass_diversity(j, cls_id):
        #                 kept.append(j)
        #         selected_idx = (torch.tensor(kept, device=confidence_prediction.device, dtype=torch.long)
        #                         if len(kept) > 0 else selected_idx)
        #
        #     # （可选）保险：如果上面没定义 selected_idx，就给个空的，保证后面的 boosting 还能走 fallback
        #     if 'selected_idx' not in locals():
        #         selected_idx = torch.tensor([], device=confidence_prediction.device, dtype=torch.long)
        #     # ================== end EuroSAT-only gating ==================
        #     # === Caltech no-boosting OR default boosting ===
        #
        #     # 判定是否 Caltech101（优先用 model.dataset_id，其次用 args.set_id）
        #     is_caltech = False
        #     if 'dataset_id' in dir(model) and isinstance(model.dataset_id, str):
        #         ds = model.dataset_id.lower()
        #         is_caltech = ('caltech101' in ds) or ('caltech' in ds)
        #     if (not is_caltech) and hasattr(args, 'set_id'):
        #         sid = str(args.set_id).lower()
        #         is_caltech = (sid == 'caltech101' or sid == 'caltech')
        #
        #     if is_caltech:
        #         # 不走 boosting，多视图不展开，直接单次写库
        #         with torch.no_grad():
        #             dmnet.update_memory_bank(model, target)
        #         # （后续流程照常继续：不会改动 selected_idx、orig_first 等变量）
        #     else:
        #         # ——原 boosting 代码保持不变——
        #         with torch.no_grad():
        #             orig_first = model.image_features_global[:1].clone()
        #
        #             if selected_idx.numel() > 0:
        #                 for t, j in enumerate(selected_idx.tolist()):
        #                     model.image_features_global[:1] = model.image_features_global[j:j + 1]
        #                     dmnet.init_pred = confidence_prediction[t:t + 1]  # [1, n_cls]
        #                     dmnet.update_memory_bank(model, target)
        #             else:
        #                 dmnet.update_memory_bank(model, target)
        #
        #             model.image_features_global[:1] = orig_first
        #             dmnet.init_pred = confidence_prediction.mean(0, keepdim=True)
        #     # === end (Caltech gated) boosting ===
        dmnet.update_memory_bank(model, target)
        with torch.no_grad():
        ...
        pred_vanilla.append(img_text_pred.detach().float().cpu())
        pred_global.append(img_global_pred.detach().float().cpu())
        if args.n_shot:
            pred_fewshot_global.append(fewshot_global_pred.detach().float().cpu())
        labels.append(target.detach().cpu())
        pred_sakr.append(sakr_pred.detach().float().cpu())
    ...
    pred_sakr = torch.cat(pred_sakr, dim=0)
    ...
    if getattr(args, 'tda', False) and len(tda_pred) > 0:
        pred_tda = torch.cat(tda_pred, dim=0)
    else:
        pred_tda = pred_vanilla
    ...
    search_step = 10
    if weight_search:
        beta1_list = [0.25, 0.5, 1.0]
        beta2_list = [0.001, 0.002, 0.003, 0.005, 0.01, 0.02, 0.03, 0.05, 0.1, 0.2,
                      0.3, 0.5, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 1000]

        if args.n_shot:
            beta3_list = [0.001, 0.002, 0.003, 0.005, 0.01, 0.02, 0.03, 0.05, 0.1, 0.2,
                          0.3, 0.5, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 1000]
        else:
            beta3_list = [0]
        beta4_list = ([0.001, 0.002, 0.003, 0.005, 0.01, 0.02, 0.03, 0.05, 0.1, 0.2,
                       0.3, 0.5, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 1000]
                      if getattr(args, 'tda', False) else [0.0])
        beta5_list = ([0.001, 0.002, 0.003, 0.005, 0.01, 0.02, 0.03, 0.05, 0.1, 0.2,
                       0.3, 0.5, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 1000]
                      if (getattr(args, 'use_sakr_branch', False) and getattr(args, 'n_shot', 0))
                      else [0.0])
        ...
        best_acc = 0.
        best_beta1 = best_beta2 = best_beta3 = best_beta4 = best_beta5 = 0.
                ...
                for beta3 in beta3_list:
                    for beta4 in beta4_list:
                        for beta5 in beta5_list:
                            logits = (pred_vanilla * beta1 +
                                      pred_global * beta2 +
                                      pred_fewshot_global * beta3 +
                                      pred_tda * beta4 +
                                      pred_sakr * beta5)
                            ...
                            if acc > best_acc:
                                print(
                                    'New best setting, beta1: {:.4f}; beta2: {:.4f}; beta3: {:.4f}; beta4: {:.4f}; beta5: {:.4f}; Acc: {:.2f}'
                                    .format(beta1, beta2, beta3, beta4, beta5, acc))
                                best_acc = acc
                                best_beta1, best_beta2, best_beta3, best_beta4, best_beta5 = beta1, beta2, beta3, beta4, beta5

        print(f"Searched Acc: {best_acc:.2f} with "
              f"beta1 {best_beta1:.3f}, dynamic {best_beta2:.3f}, static {best_beta3:.3f}, "
              f"tda {best_beta4:.3f}, sakr {best_beta5:.3f}")

        return [top1.avg, top1_global.avg, top1_global_fewshot.avg,
                best_acc, best_beta1, best_beta2, best_beta3, best_beta4, best_beta5]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_bridge_branch', action='store_true',
                        help='i2t')
    parser.add_argument('--bridge_w', type=float, default=0.0,
                        help='')
    

    parser.add_argument('--tda', action='store_true', default=False,
                        help='')
    parser.add_argument('--tda_pos_k', type=int, default=3, help='正缓存每类最大条目')
    parser.add_argument('--tda_neg_k', type=int, default=2, help='负缓存每类最大条目')
    parser.add_argument('--tda_pos_alpha', type=float, default=1.0, help='正缓存 α')
    parser.add_argument('--tda_pos_beta', type=float, default=5.0, help='正缓存 β')
    parser.add_argument('--tda_neg_alpha', type=float, default=1.0, help='负缓存 α')
    parser.add_argument('--tda_neg_beta', type=float, default=5.0, help='负缓存 β')
    parser.add_argument('--tda_neg_entropy_low', type=float, default=0.2, help='负缓存熵门 τ_l')
    parser.add_argument('--tda_neg_entropy_high', type=float, default=0.5, help='负缓存熵门 τ_h')
    parser.add_argument('--tda_neg_mask_low', type=float, default=0.03, help='负掩码下界 p_l')
    parser.add_argument('--tda_neg_mask_high', type=float, default=0.30, help='负掩码上界 p_h')

    parser.add_argument('--use_sakr_branch', action='store_true', default=False,
                        help='sakr')
    parser.add_argument('--sakr_beta', type=float, default=5.0,
                        help='sakr RBF beta')
    parser.add_argument('--sakr_lambda', type=float, default=1.0,
                        help='sakr KRR lambda')
