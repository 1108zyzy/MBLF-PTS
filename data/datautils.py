...
def build_dataset(set_id, transform, data_root, mode='test', n_shot=None, split="all", bongard_anno=False):
    ...
    return testset  # 返回：testset（或 few-shot 情况下的 supportset）
...
# 返回 多视图（multi-views）：1 个“基准视图” + n 个 AugMix 视图
...
class AugMemAugmenter(object):
    ...
    def __call__(self, x):
        ...
        return [image] + views  # views 是给“记忆样本”用的弱增强试图
...
class StrongAugmenterRand(object):
    ...
    # 随机返回“弱增强视图”或“AugMix 视图”（各 50% 概率），实现“有时增强、更鲁棒”的输入
    def __call__(self, x):
